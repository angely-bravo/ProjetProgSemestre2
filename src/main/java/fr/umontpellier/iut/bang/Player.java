...
public class Player {
    ...
    /**
     * Verifie si le joueur a jouer ou non un Bang!
     */
    ...
    public List<Player> getOtherPlayers() {
        ...
        int indexPlayer = (getGame().getPlayers().indexOf(this) + 1) % game.getPlayers().size();
        for (int i = 0; i < game.getPlayers().size(); i++) {
            result.add(game.getPlayers().get(indexPlayer));
            indexPlayer = (indexPlayer + 1) % game.getPlayers().size();
        }
        result.remove(this);
        return result;
    }
    ...
    public List<Player> getPlayersInRange(int range) {
        ...
        for (Player p : getOtherPlayers()) {
            ...
            if (getCardInPlay("Scope") != null) {
                distance--;
            }

            if (p.getCardInPlay("Mustang") != null) {
                distance++;
            }

            if (p.getBangCharacter().getClass().getName().equals(PaulRegret.class.getName())) {
                distance++;
            }
            if (bangCharacter.getClass().getName().equals(RoseDoolan.class.getName())) {
                distance--;
            }
    ...
    public Card getCardsInHand(String cardName) {
        for (Card c : hand) {
            if (c.getName().equals(cardName)) {
                return c;
            }
    ...
    public void decrementHealth(int n, Player attacker) {
        ...
        if (!isDead()) {
            if (bangCharacter.getClass().getName().equals(BartCassidy.class.getName())) {
                for (int i = 0; i < n; i++) {
                    drawToHand();
                }
            }
            if (attacker != null) {
                if (attacker.getBangCharacter().getName().equals("Suzy Lafayette")) {
                    while (attacker.getHand().size() < 1) {
                        attacker.getHand().add(attacker.drawCard());
                    }
                }

                if (attacker != null && bangCharacter.getClass().getName().equals(ElGringo.class.getName()) && attacker.getHand().size() != 0) {
                    for (int i = 0; i < n; i++) {
                        hand.add(attacker.removeRandomCardFromHand());
                    }
                }
            }
        }
    ...
    public void ifIsDead(Player attacker) {
        if (isDead()) {
            if (game.getPlayers().size() > 2) {
                boolean containsBeer = true;


                while (healthPoints < 1 && containsBeer && hand.size() > 0) {
                    if (getCardsInHand("Beer") != null) {
                        List<Card> biere = new ArrayList<>();
                        biere.add(getCardsInHand("Beer"));
                        Card pass; //Represente le choix du joueur
                        pass = chooseCard("Sauvez vous grace a des bières !!!", biere, true, true);
                        if (pass == null) {
                            containsBeer = false;
                        } else {
                            pass.playedBy(this);
                        ...
                        containsBeer = false; //casse le While
                    }
                }
            ...
            if (isDead()) {
                ...
                if (attacker != null && getRole() == Role.DEPUTY && attacker.getRole() == Role.SHERIFF) {
                    List<Card> main = new ArrayList<>();
                    main.addAll(attacker.getHand());
                    for (Card c : main) {
                        attacker.discardFromHand(c);
                    }

                }

                for (Player p : getOtherPlayers()) {
                    if (p.getBangCharacter().getClass().getName().equals(VultureSam.class.getName())) {
                        p.getHand().addAll(hand);
                        p.getHand().addAll(inPlay);
                        for (int i = 0; i < hand.size(); ) {
                            hand.remove(hand.get(i));
                        }
                        for (int i = 0; i < hand.size(); ) {
                            inPlay.remove(inPlay.get(i));
                        }
                        break;
                    }
                }
                ...
                if (inPlay.size() != 0) {
                    for (int i = 0; i < inPlay.size(); ) {
                        removeFromInPlay(inPlay.get(i));
                    }
                }
                if (game.getOutlawPlayers().contains(this) && attacker != null) {
                    attacker.drawToHand();
                    attacker.drawToHand();
                    attacker.drawToHand();
                }
    ...
    public int sauvetage(Player attacker) {
        int sauvetage = 1;

        if (attacker != null) {
            if (attacker.getBangCharacter().getClass().getName().equals(SlabTheKiller.class.getName())) {
                sauvetage = 2;
            }
        }

        if (bangCharacter.getClass().getName().equals(Jourdonnais.class.getName())) {
            if (degaineSuit().getSuit() == CardSuit.HEART) {
                sauvetage--;
            }
        }
        ...
        b:
        {
            if (sauvetage == 0) break b;

            if (getCardInPlay("Barrel") != null) {
                if (degaineSuit().getSuit() == CardSuit.HEART) {
                    sauvetage--;
                }
            }
    ...
    public int useMissed(int sauvetage) {
        c:
        {
            if (sauvetage == 0) break c;


            List<Card> missed = new ArrayList<>();

            for (Card c : hand) {
                if (c.getClass().getName().equals(Missed.class.getName())) {
                    missed.add(c);
                }
                if (c.getClass().getName().equals(Bang.class.getName()) && (bangCharacter.getClass().getName().equals(CalamityJanet.class.getName()))) {
                    missed.add(c);
                }
            }
            while (sauvetage > 0) {
                Card choisis = chooseCard("Sauvez vous !!!", missed, true, true);
                if (choisis != null) {
                    sauvetage--;
                    missed.remove(choisis);
                    discardFromHand(choisis);
                } else {
                    break;
                }
            }
    ...
    public int useBang(int sauvetage) {
        c:
        {
            if (sauvetage == 0) break c;
            ...
            for (Card c : hand) {
                if (c.getClass().getName().equals(Bang.class.getName())) {
                    missed.add(c);
                }
                if (c.getClass().getName().equals(Missed.class.getName()) && (bangCharacter.getClass().getName().equals(CalamityJanet.class.getName()))) {
                    missed.add(c);
                }
            }
            while (sauvetage > 0) {
    ...
    public int distanceTo(Player player) {
        ...
        if (player == this) {
            ...
            int r = game.getPlayerDistance(this, player);
            return r;
        }
    }
    ...
    public void playFromHand(Card card) {
        ...
        for (Player p : game.getPlayers()) {
            if (p.getBangCharacter().getClass().getName().equals(SuzyLafayette.class.getName())) {
                while (p.getHand().size() < 1) {
    ...
    public void playTurn() {
        ...
        //Made by Louis && Angely
        List<BlueCard> inPlay = new ArrayList<>();
        inPlay.addAll(this.inPlay);
        boolean prison = false;


        for (BlueCard bc : inPlay) {
            if (bc.getClass().getName() == Dynamite.class.getName()) {
                ((Dynamite) bc).preliminaryEffect(this);
                if (isDead()) {
                    prison = true;
                }
            }
        ...
        s:
        {
            if (prison) break s;
            for (BlueCard bc : inPlay) {
                if (bc.getClass().getName() == Jail.class.getName()) {
                    prison = ((Jail) bc).preliminaryEffect(this);
                }
            }
            ...
            while (true) {
                ...
                if (card == null) break;
                if (card.getName() == Bang.class.getName()) {
                    if (bang || bangCharacter.getClass().getName() == WillyTheKid.class.getName() || getWeapon().getClass().getName() == Volcanic.class.getName()) {
                        playFromHand(card);
                    } else {
                        continue;
                    }
    ...
    public Card degaine() {
        List<Card> choix = new ArrayList<>();
        if (bangCharacter.getClass().getName() == LuckyDuke.class.getName()) {

            for (int i = 0; i < 2; i++) {
                choix.add(randomDraw());
            }
            return chooseCard("Choisissez une carte !", choix, true, false);
        } else {
            Card r = randomDraw();
            return r;
        }
    ...
    public Card degaineSuit() {
        List<String> choix = new ArrayList<>();
        List<Card> carteChoix = new ArrayList<>();
        if (bangCharacter.getClass().getName() == LuckyDuke.class.getName()) {
            for (int i = 0; i < 2; i++) {
                carteChoix.add(randomDraw());
            }
            for (Card c : carteChoix) {
                choix.add(c.getPokerString());
            }
            String r = choose("Choisissez une carte !", choix, true, false);
            if (carteChoix.get(0).getSuit().toString().equals(r)) {
                return carteChoix.get(0);
            } else {
                return carteChoix.get(1);
            }
