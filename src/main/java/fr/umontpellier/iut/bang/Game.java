...
public class Game {
    ...
    public int getPlayerDistance(Player player1, Player player2) {
        ...
        int indicePlayer1 = 0,indicePlayer2 = 0,distance;

        for (int i = 0 ; i< getPlayers().size();i++){
            if (getPlayers().get(i).equals(player1)){
                indicePlayer1 = i;
                break;
            }
        }
        ...
        if(indicePlayer2>indicePlayer1){
            if (indicePlayer2-indicePlayer1>getPlayers().size()/2){
                distance = getPlayers().size()-(indicePlayer2-1);
            }
            else{
                distance = indicePlayer2-indicePlayer1;
            }
        }
        else{
            if (indicePlayer1-indicePlayer2>getPlayers().size()/2){
                distance = getPlayers().size()-(indicePlayer1-1);
            }
            else{
                distance = indicePlayer1-indicePlayer2;
            }
        }
    ...
    private void updateGameFinished() {
        if(!players.contains(sheriffPlayer)){
            if(players.size()==1 && players.contains(renegadePlayer)){
            ...
            if (!(players.containsAll(outlawPlayers) && players.contains(renegadePlayer))){
    ...
    public List<Player> getOutlawPlayers() {
